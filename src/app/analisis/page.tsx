...
// eslint-disable-next-line react-hooks/exhaustive-deps
 ...
 const validateStep1PreRequisites = (): { isValid: boolean, message?: string } => {
  ...
  const handleVerifyEfficacy = async (comments: string) => {
    ...
    const saveResult = await handleSaveAnalysisData(
        false, 
        { efficacyVerificationOverride: efficacyUpdate, statusOverride: "Verificado" }
    );
     if (saveResult.success) {
      toast({ title: "Eficacia Verificada", description: "El análisis ha sido marcado como verificado y completado." });
    } else {
      toast({ title: "Error al Verificar", description: "No se pudo guardar el estado de verificación.", variant: "destructive" });
      // Revert local state on failure
      setEfficacyVerification(prev => ({...prev, status: 'pending'}));
    }
    setIsSaving(false);
  };
  ...
  const handlePlanEfficacyVerification = async (responsible: string, verificationDate: string) => {
    ...
    try {
        ...
        if (saveResult.success) {
            setEfficacyVerification(efficacyUpdate);
            toast({ title: "Verificación Planificada", description: "Se ha establecido la fecha para la verificación de eficacia." });
        } else {
            toast({ title: "Error al Planificar", description: "No se pudo guardar la fecha de planificación." + (saveResult.needsNavigationUrl ? " Navegación pendiente, recargue." : ""), variant: "destructive" });
        }
    } catch (error) {
        console.error("Error al planificar verificación de eficacia:", error);
        toast({ title: "Error", description: `Ocurrió un error: ${(error as Error).message}`, variant: "destructive" });
    } finally {
        setIsSaving(false);
    }
  };
