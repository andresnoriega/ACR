    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Por favor, ingrese el método utilizado para validar o rechazar esta línea de investigación.
          </DialogDescription>
        </DialogHeader>
        <div className="py-4">
...
const getNodeByPath = (data: FiveWhyEntry[], path: (string | number)[]): any => {
    ...
    for (const key of path) {
        ...
        if (typeof key === 'string' && key === 'subAnalysis' && typeof current === 'object') {
            ...
            current = current[key];
        } else if (typeof key === 'number' && Array.isArray(current.responses)) { // Handle path like [0, 'responses', 0]
...
const getParentArrayAndIndex = (data: FiveWhyEntry[], path: (string | number)[]): { parentArray: any[], index: number } | null => {
    ...
    for (let i = 0; i < path.length - 1; i++) {
        ...
        if (typeof key === 'number' && Array.isArray(parent)) {
            parent = parent[key];
        } else if (typeof key === 'string' && typeof parent === 'object' && parent !== null && key in parent) {
            parent = parent[key];
        } else if (typeof key === 'number' && typeof parent === 'object' && parent !== null && 'responses' in parent && Array.isArray(parent.responses)) {
            parent = parent.responses[key];
        } else {
    ...
    // Check if parent is the array we want or contains the array
    if (Array.isArray(parent)) {
      return { parentArray: parent, index: indexToRemove };
    }
    if (parent && Array.isArray(parent.responses)) {
      return { parentArray: parent.responses, index: indexToRemove };
    }
...
export const FiveWhysInteractive: FC<FiveWhysInteractiveProps> = ({
  ...
  useEffect(() => {
    if (fiveWhysData.length === 0) {
      onSetFiveWhysData([
        { id: generateStableId('why'), why: initialWhyText, responses: [] }
      ]);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fiveWhysData, initialWhyText]);

 const handleUpdate = (path: (string|number)[], value: any, field: keyof FiveWhyNode | 'why' | 'isCollapsed' | 'width') => {
  ...
  const handleSetRootCause = (path: (string | number)[]) => {
    const newData = JSON.parse(JSON.stringify(fiveWhysData));
  ...
  const confirmSetRootCause = () => {
    ...
    const nodeToUpdate = getNodeByPath(newData, path);
    if(nodeToUpdate){
      nodeToUpdate.isRootCause = !nodeToUpdate.isRootCause; // Toggle
      if (nodeToUpdate.isRootCause && nodeToUpdate.status !== 'accepted') {
  ...
  const handleAddWhyInvestigation = () => {
    onSetFiveWhysData([
      ...fiveWhysData,
      { id: generateStableId('why'), why: `¿Por qué ocurrió el evento? (Investigación Paralela #${fiveWhysData.length})`, responses: [] }
    ]);
  };
