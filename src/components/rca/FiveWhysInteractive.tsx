...
import type { CTMData, FailureMode, Hypothesis, PhysicalCause, HumanCause, LatentCause, FiveWhysData } from '@/types/rca';
...
export const FiveWhysInteractive: FC<FiveWhysInteractiveProps> = ({ fiveWhysData, onSetFiveWhysData }) => {
  ...
  const handleAdd = (path: (string | number)[]) => {
    ...
    if (lastKey === null) { // Adding a new FailureMode to the root
      newData.push({ id: generateId('fm'), description: 'Nuevo ¿Por qué?', hypotheses: [] });
    } else {
      let targetArray;
      ...
      if ('latentCauses' in parent) {
        ...
        parent.hypotheses.push({ id: generateId('hyp'), description: 'Nuevo Porque...', physicalCauses: [], status: 'pending' });
      }
    }
        ...
        <div className="flex space-x-4 overflow-x-auto py-2">
          ...
          {fiveWhysData.length === 0 && (
            <div className="text-center text-muted-foreground italic py-4 w-full">
              Haga clic en "Añadir ¿Por qué?" para comenzar a construir el análisis.
            </div>
          )}
        </div>
      </div>
