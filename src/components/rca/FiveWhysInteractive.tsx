...
// Tipos base
type FiveWhyStatus = 'pending' | 'accepted' | 'rejected';

interface FiveWhysCause {
  id: string;
  description: string;
  status: FiveWhyStatus;
...
type FiveWhysData = FiveWhysEntry[];
...
// Esta versión solo filtra causas y entradas nulas o con id vacío, pero permite descripciones vacías.
function cleanFiveWhysData(data: FiveWhysData): FiveWhysData {
  return (data || [])
    .filter(entry =>
      entry &&
      typeof entry.id === 'string' && entry.id &&
      typeof entry.why === 'string' && entry.why &&
      Array.isArray(entry.becauses) && entry.becauses.length > 0
    )
    .map(entry => ({
      id: entry.id,
      why: entry.why,
      becauses: entry.becauses
        .filter(c =>
          c &&
...
export const FiveWhysInteractive: FC<FiveWhysInteractiveProps> = ({
  ...
  // Múltiples análisis
  const [analyses, setAnalyses] = useState<{ id: string; data: FiveWhysData }[]>(() => {
  ...
  // Actualiza un análisis específico (aplica limpieza antes de guardar/enviar)
  const updateAnalysis = useCallback((idx: number, newData: FiveWhysData) => {
    try {
      const checkedData = cleanFiveWhysData(newData);
      setAnalyses(prev =>
  ...
  // --- LOG PARA DEPURAR ---
  // Puedes poner este log antes de enviar el payload al backend/IA
  // console.log("FiveWhysData que se envía:", JSON.stringify(cleanFiveWhysData(analyses[0].data), null, 2));
